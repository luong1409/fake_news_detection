...
class RobertaFakeNewsClassification(RobertaPreTrainedModel):
    """Custom Roberta Base model

    Args:
        RobertaPreTrainedModel (Parent Class): this is Pretrained class from transformers module
    """
    def __init__(self, config: BertConfig) -> None:
        super().__init__(config)
        self.roberta = RobertaModel(config)
        self.num_labels = config.num_labels
        self.outputs = nn.Linear(in_features=config.hidden_size * 4, out_features=self.num_labels)
...
class RobertaLargeFakeNewsClassification(RobertaPreTrainedModel):
    """
